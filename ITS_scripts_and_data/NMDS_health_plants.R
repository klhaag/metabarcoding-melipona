...
# composition according to health status

library(tidyverse)
library(readxl)
library(ggtext)

nmds <- read_tsv(file="raw_data_plants/ITS_plants_only.agc.unique_list.0.05.filter.0.05.subsample.braycurtis.0.05.square.nmds.axes")
...
ggplot(metadata_nmds,
       aes(x=axis1, y=axis2, color=disease, fill=disease)) +
