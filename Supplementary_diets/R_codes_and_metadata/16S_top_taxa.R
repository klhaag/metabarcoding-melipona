...
taxonomy <- read_tsv(file="16SB.taxonomy") %>%
  rename_all(tolower) %>%
  mutate(taxonomy=str_replace_all(string=taxonomy, pattern="\\(\\d*\\)", replacement="")) %>%
  mutate(taxonomy=str_replace_all(string=taxonomy, pattern=";$", replacement="")) %>%
  separate(taxonomy, into=c("kingdom", "phylum", "class", "order", "family", "genus"), sep=";")

otu_data <- read_tsv("16SB.subsample.filter.shared", 
...
agg_genus_metadata %>%
  ...
  scale_color_manual(name=NULL,
                     ...
                     labels=c("AAL", "DAL", "ASA", "DSA", "ACT", "DCT")) +
  scale_fill_manual(name=NULL,
...
#COMPARING FAMILIES PERTREATMENT

metadata <- read_excel(file="metadata_supplement.xlsx", range = cell_cols(1:5),
                       col_types=c(sample = "text", colony = "text", period = "text", treatment = "text", pertreatment = "text"))
...
agg_family_metadata %>%
  ...
  mutate(agg_rel_abund = agg_rel_abund + 1/21000) %>%
  mutate(family=factor(family, levels=top_taxa)) %>%
  mutate(pertreatment=factor(pertreatment, levels=c("DCT", "ACT", "DSA", "ASA", "DAL", "AAL"))) %>%
  ggplot(aes(x=family, y=agg_rel_abund, color=pertreatment, fill=pertreatment)) +
  geom_boxplot(alpha=0.9, outlier.shape=NA) +
  scale_color_manual(name=NULL,
                     values=c("darkblue", "darkred", "darkgreen", "darkorange", "darkblue", "darkred"),
                     breaks=c("AAL", "DAL", "ASA", "DSA", "ACT", "DCT"),
