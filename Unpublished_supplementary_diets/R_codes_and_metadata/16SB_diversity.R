...
alpha <- read_tsv(file="16SB.alfadiversity.ave-std.summary",
                  col_types=cols(group = col_character())) %>%
  filter(method=='ave') %>%
  select(group, sobs, shannon, shannoneven, chao, invsimpson, coverage)
...
# plotting chao index x treatment x period

ggplot(meta_alpha, aes(x=treatment, y=chao, color=period, fill=period)) +
  geom_boxplot(alpha=0.9, outlier.colour=NA) +
  geom_jitter(shape=19)+
  scale_color_manual(name=NULL,
                     values=c("darkblue","darkred"),
...
ggplot(meta_alpha, aes(x=period, y=shannon, color=period, fill=period)) +
  ...
  scale_fill_manual(name=NULL,
                    ...
                    breaks=c("A","D"),
                    labels=c("Before","After")) +
  labs(title="Shannon Index per period",
       x=NULL,
       y="Shannon Index") +
  theme_classic()
...
meta_alpha %>%
  select(sample, chao, colony, period, treatment, pertreatment) %>%
  pivot_longer(cols=c(colony, period, treatment), names_to="characteristic", values_to="value") %>%
  drop_na() %>%
  nest(data = -characteristic) %>%
  mutate(tests = map(data, ~tidy(kruskal.test(chao ~ value, data=.x)))) %>%
  unnest(cols=tests) %>%
  select(-data) %>%
  mutate(p.value.adj = p.adjust(p.value, method="BH"))
