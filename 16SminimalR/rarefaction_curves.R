...
rarefy <- read_tsv(file="raw_data/melipona16s.phylip.an.groups.rarefaction") %>%
...
metadata <- read_excel(path="raw_data/metadata.xlsx", col_names=TRUE)

metadata_rarefy <- inner_join(metadata, rarefy, c("sample"))

ggplot(metadata_rarefy, aes(x=numsampled, y=sobs.y, color=colony, group=sample)) +
    geom_line() +
